...
def handle_special_texture(fname, shape, rng, probe=False):
    """
...
def can_generate(fname: str, shape=None) -> bool:
    ...
    if fname in name_to_text_lut:
        return True
    test = generate_semantic_image_data(fname, shape or (32, 32, 4), None, None, probe=True)
    return test


def generate_semantic_image_data(fname, shape, rng, identity=None, probe=False):
    return handle_special_texture(fname, shape, rng, probe=probe)
